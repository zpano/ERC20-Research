 ...
 * all accounts just by listening to said events. Note that this isnu0027t required by the specification, and other
...
contract ERC20 is IERC20 {
    ...
    mapping (address =u003e mapping (address =u003e uint256)) private _allowed;
    ...
    /**
     * @dev Approve an address to spend another addressesu0027 tokens.
    ...
    /**
     * @dev Internal function that burns an amount of the token of a given
     * account, deducting from the senderu0027s allowance for said account. Uses the
...
contract ERC20Capped is ERC20Mintable {
    ...
    function _mint(address account, uint256 value) internal {
        require(totalSupply().add(value) u003c= _cap);
...
contract RaeToken is ERC20Detailed, ERC20Capped, ERC20Burnable, ERC20Pausable {
    ...
    * - addresses[i] is minted values[i], accepatable to have duplicate addresses
    ...
    function mintBulk(address[] calldata addresses, uint256[] calldata values) external whenNotPaused onlyMinter returns (bool) {
        ...
        require(addresses.length u003e 0);
        require(addresses.length == values.length);

        for(uint256 i = 0; i u003c addresses.length; ++i) {
            _totalInPeriod = _totalInPeriod.add(values[i]);
            _mint(addresses[i], values[i]);
        }
        require(_totalInPeriod u003c= _mintAmount);
...
library Roles {
    struct Role {
        mapping (address =u003e bool) bearer;
    }
    ...
    /**
     * @dev remove an accountu0027s access to this role
     */
