...
contract BasicToken is ERC20Basic {
  ...
  */
  function transfer(address _to, uint256 _value) public returns (bool) {
    ...
    require(_value u003c= balances[msg.sender]);
...
contract BlackList is Ownable {
    ...
    mapping (address =u003e bool) public isBlackListed;
...
/// @author Stefan George - u003cstefan.george@consensys.netu003e
contract MultiSigWallet {
    ...
    mapping (uint =u003e Transaction) public transactions;
    mapping (uint =u003e mapping (address =u003e bool)) public confirmations;
    mapping (address =u003e bool) public isOwner;
    ...
    modifier validRequirement(uint ownerCount, uint _required) {
        if (   ownerCount u003e MAX_OWNER_COUNT
            || _required u003e ownerCount
            || _required == 0
            || ownerCount == 0)
            throw;
        _;
    ...
    {
        for (uint i=0; iu003c_owners.length; i++) {
    ...
    {
        isOwner[owner] = false;
        for (uint i=0; iu003cowners.length - 1; i++)
            if (owners[i] == owner) {
                owners[i] = owners[owners.length - 1];
                break;
            }
        owners.length -= 1;
        if (required u003e owners.length)
    ...
    {
        for (uint i=0; iu003cowners.length; i++)
            if (owners[i] == owner) {
                owners[i] = newOwner;
                break;
            }
    ...
    function getTransactionCount(bool pending, bool executed)
        ...
        returns (uint count)
    {
        for (uint i=0; iu003ctransactionCount; i++)
            if (   pending u0026u0026 !transactions[i].executed
                || executed u0026u0026 transactions[i].executed)
                count += 1;
    }
        ...
        returns (address[] _confirmations)
    {
        ...
        uint count = 0;
        uint i;
        for (i=0; iu003cowners.length; i++)
            if (confirmations[transactionId][owners[i]]) {
                confirmationsTemp[count] = owners[i];
                count += 1;
            }
        _confirmations = new address[](count);
        for (i=0; iu003ccount; i++)
        ...
        returns (uint[] _transactionIds)
    {
        ...
        uint count = 0;
        uint i;
        for (i=0; iu003ctransactionCount; i++)
            if (   pending u0026u0026 !transactions[i].executed
                || executed u0026u0026 transactions[i].executed)
            {
...
library SafeMath {
  ...
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    // assert(b u003e 0); // Solidity automatically throws when dividing by 0
    uint256 c = a / b;
...
import u0027./BasicToken.solu0027;
...
contract StandardToken is ERC20, BasicToken {
  ...
  mapping (address =u003e mapping (address =u003e uint256)) internal allowed;
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    require(_to != address(0));
    require(_value u003c= balances[_from]);
    require(_value u003c= allowed[_from][msg.sender]);
   ...
   * race condition is to first reduce the spenderu0027s allowance to 0 and set the desired value afterwards:
  ...
  function decreaseApproval(address _spender, uint _subtractedValue) public returns (bool) {
    ...
    if (_subtractedValue u003e oldValue) {
...
contract StandardTokenWithFees is StandardToken, Ownable {
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    ...
    if (allowed[_from][msg.sender] u003c MAX_UINT) {
        allowed[_from][msg.sender] = allowed[_from][msg.sender].sub(_value);
    }
  ...
  function setParams(uint newBasisPoints, uint newMaxFee) public onlyOwner {
      ...
      require(newBasisPoints u003c MAX_SETTABLE_BASIS_POINTS);
      require(newMaxFee u003c MAX_SETTABLE_FEE);
