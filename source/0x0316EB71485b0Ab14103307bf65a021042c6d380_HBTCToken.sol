...
contract Container{
    ...
    mapping (bytes32 =u003e Item) private container;

    function itemAddressExists(bytes32 id, address oneAddress) internal view returns(bool){
        for(uint256 i = 0; i u003c container[id].addresses.length; i++){
    ...
    function addItemAddress(bytes32 id, address oneAddress) internal{
        ...
        require(container[id].addresses.length u003c MaxItemAdressNum, "too many addresses");
...
contract HBTCAdmin is Container{
    ...
    function classHashExist(bytes32 aHash) private view returns(bool){
        for(uint256 i = 0; i u003c classHashArray.length; i++)
    ...
    function resetRequiredNum(string memory class, uint256 requiredNum)
        ...
        if (classHash == OWNERHASH)
            require(requiredNum u003c= getItemAddressCount(OWNERHASH),"num larger than existed owners");

        bytes32 taskHash = keccak256(abi.encodePacked("resetRequiredNum", class, requiredNum));
        addItemAddress(taskHash, msg.sender);

        if(getItemAddressCount(taskHash) u003e= ownerRequireNum){
            removeItem(taskHash);
    ...
    function dropAddress(string memory class, address oneAddress)
        public onlyOwner returns(bool){
        bytes32 classHash = getClassHash(class);
        require(classHash != STOREHASH u0026u0026 classHash != LOGICHASH, "wrong class");
        require(itemAddressExists(classHash, oneAddress), "no such address exist");

        if(classHash == OWNERHASH)
            require(getItemAddressCount(classHash) u003e ownerRequireNum, "insuffience addresses");
...
contract HBTCLogic {
    ...
    function mintLogic(uint256 value,address to,string calldata proof,
        ...
        require(value u003e 0, "value need u003e 0");
    ...
    function burnLogic(address from, uint256 value,string calldata btcAddress,
        ...
        require(balance u003e= value,"sender address not have enough HBTC");
        require(value u003e 0, "value need u003e 0");
    ...
    function transferLogic(address sender,address to,uint256 value) external onlyCaller returns(bool) {
        ...
        uint256 balanceTo = store.balanceOf(to);
        require(value u003c= balanceFrom, "insufficient funds");
    ...
    function transferFromLogic(address sender,address from,address to,uint256 value) external onlyCaller returns(bool) {
        ...
        require(to != address(0), "cannot transfer to address zero");
        require(value u003e 0, "can not tranfer zero Token");
        ...
        require(value u003c= allowedvalue, "insufficient allowance");
        require(value u003c= balanceFrom, "insufficient funds");
    ...
    function cancelTask(bytes32 taskHash)  external onlyCaller returns(uint256){
        ...
        if(theSupporterNum u003e 0) store.removeAllSupporter(taskHash);
...
contract HBTCStorage is Container{
    ...
    mapping (address =u003e uint256) private balances;

    mapping (address =u003e mapping (address =u003e uint256)) private allowed;
    ...
    function setAllowed(address owner,address spender,uint256 amount) external onlyCaller {
        ...
        require(amount u003c= balances[owner], "owner balance need u003e= amount");
...
}"},"Migrations.sol":{"content":"pragma solidity u003e=0.4.21 u003c0.6.0;
...
}"},"SafeMath.sol":{"content":"// solium-disable linebreak-styler
pragma solidity ^0.5.11;r
r
library SafeMath {r
