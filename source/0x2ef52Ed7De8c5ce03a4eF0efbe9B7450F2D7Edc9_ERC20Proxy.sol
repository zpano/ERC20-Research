...
/** @title  A dual control contract.
  ...
  * Requests made by a u0027primaryu0027 account have a default time lock applied.
...
contract Custodian {
    ...
    mapping (address =u003e bool) public signerSet;

    /// @dev  The map of request hashes to pending requests.
    mapping (bytes32 =u003e Request) public requestMap;

    /// @dev  The map of callback addresses to callback selectors to request indexes.
    mapping (address =u003e mapping (bytes4 =u003e uint256)) public lastCompletedIdxs;
        ...
        public
    {
        ...
        require(_signers.length u003e= 2, "at least two `_signers`");

         // validate time lock params
        require(_defaultTimeLock u003c= _extendedTimeLock, "valid timelock params");
        ...
        for (uint i = 0; i u003c _signers.length; i++) {
            // no zero addresses or duplicates
            require(_signers[i] != address(0) u0026u0026 !signerSet[_signers[i]], "no zero addresses or duplicates");
    ...
    /** @notice  Requests an unlocking with a lock identifier and a callback.
      ...
      * and the lock identifier parameters a u0027whitelisted addressu0027 is required
    ...
    */
    ...
    {
        require(msg.sender == primary || msg.value u003e= 1 ether, "sender is primary or stake is paid");
    ...
    */
        ...
        returns (bool success)
    {
        ...
        require(request.idx u003e lastCompletedIdxs[callbackAddress][callbackSelector],
        "reject confirms of earlier withdrawals buried under later confirmed withdrawals");
...
32", _requestMsgHash)),
        ...
        if (request.extended u0026u0026 ((block.timestamp - request.timestamp) u003c extendedTimeLock)) {
            emit TimeLocked(request.timestamp + extendedTimeLock, _requestMsgHash);
            return false;
        } else if ((block.timestamp - request.timestamp) u003c defaultTimeLock) {
            emit TimeLocked(request.timestamp + defaultTimeLock, _requestMsgHash);
            return false;
        } else {
            if (address(this).balance u003e 0) {
    ...
    */
    function deleteUncompletableRequest(bytes32 _requestMsgHash) public {
        ...
        require(0 u003c idx u0026u0026 idx u003c lastCompletedIdxs[request.callbackAddress][request.callbackSelector],
...
contract CustodianUpgradeable is LockRequestable {
    ...
    mapping (bytes32 =u003e CustodianChangeRequest) public custodianChangeReqs;
...
contract ERC20ImplUpgradeable is CustodianUpgradeable  {
    ...
    mapping (bytes32 =u003e ImplChangeRequest) public implChangeReqs;
...
contract ERC20Store is ERC20ImplUpgradeable {
    ...
    mapping (address =u003e uint256) public balances;

    /// @dev  The mapping of allowances.
    mapping (address =u003e mapping (address =u003e uint256)) public allowed;
    ...
    /** @notice Adds `_balanceIncrease` to `_owner`u0027s balance.
      *
      * @dev  Intended for use by token implementation functions
      * that update balances. The only authorized caller
      * is the active implementation.
...
/** @title  ERC20 compliant token intermediary contract holding core logic.
  ...
  * 4. Delegated transfer control (u0027sweepingu0027).
  *
  */
contract ERC20Impl is CustodianUpgradeable {
    ...
    /// @dev  The sole authorized caller of delegated transfer control (u0027sweepingu0027).
    ...
    /** @dev  The mapping that stores whether the address in question has
      ...
      */
    mapping (address =u003e bool) public sweptSet;

    /// @dev  The map of lock ids to pending token increases.
    mapping (bytes32 =u003e PendingPrint) public pendingPrintMap;

    /// @dev The map of blocked addresses.
    mapping (address =u003e bool) public blocked;
        ...
        returns (bool success)
    {
        ...
        require(newAllowance u003e= currentAllowance, "new allowance must not be smaller than previous");
        ...
        returns (bool success)
    {
        ...
        require(newAllowance u003c= currentAllowance, "new allowance must not be smaller than previous");
    ...
    /** @notice  Burns the specified value from the senderu0027s balance.
      *
      * @dev  Senderu0027s balanced is subtracted by the amount they wish to burn.
      *
    ...
    function burn(uint256 _value) public returns (bool success) {
        ...
        require(_value u003c= balanceOfSender, "disallow burning more, than amount of the balance");
    ...
    /** @notice  Enables the delegation of transfer control for many
      ...
      * @param  _rs  The array of u0027Ru0027 components of the ECDSA signatures.
      * @param  _ss  The array of u0027Su0027 components of the ECDSA signatures.
    ...
    function enableSweep(uint8[] memory _vs, bytes32[] memory _rs, bytes32[] memory _ss, address _to) public onlySweeper {
        ...
        require((_vs.length == _rs.length) u0026u0026 (_vs.length == _ss.length), "_vs[], _rs[], _ss lengths are different");

        uint256 numSignatures = _vs.length;
        uint256 sweptBalance = 0;

        for (uint256 i = 0; i u003c numSignatures; ++i) {
...
32",sweepMsg)), _vs[i], _rs[i], _ss[i]);
    ...
    function replaySweep(address[] memory _froms, address _to) public onlySweeper {
        ...
        for (uint256 i = 0; i u003c lenFroms; ++i) {
        ...
        returns (bool success)
    {
        ...
        require(_value u003c= balanceOfFrom, "insufficient funds on `_from` balance");

        uint256 senderAllowance = erc20Store.allowed(_from, _sender);
        require(_value u003c= senderAllowance, "insufficient allowance amount");
        ...
        returns (bool success)
    {
        ...
        require(_value u003c= balanceOfSender, "insufficient funds");
        ...
        returns (bool success)
    {
        ...
        if(_value u003c= balanceOfSender) {
...
/** @title  A contact to govern hybrid control over increases to the token supply and managing accounts.
  ...
  * (the u0027controlleru0027) to increase the token supply up to a ceiling,
...
contract Controller is LockRequestable {
    ...
    /** @dev  The maximum that the token supply can be increased to
      ...
      * ceiling is what is available to the u0027controlleru0027 account.
    ...
    mapping (bytes32 =u003e PendingCeilingRaise) public pendingRaiseMap;

    /// @dev  The map of lock ids to pending wipes.
    mapping (bytes32 =u003e wipeAddress[]) public pendingWipeMap;

    /// @dev  The map of lock ids to pending force transfer requests.
    mapping (bytes32 =u003e forceTransferRequest) public pendingForceTransferRequestMap;
    ...
    function limitedPrint(address _receiver, uint256 _value) public onlyController {
        ...
        require(newTotalSupply u003e= totalSupply, "new total supply overflow");
        require(newTotalSupply u003c= totalSupplyCeiling, "total supply ceiling overflow");
    ...
    function requestWipe(address[] memory _froms, uint256[] memory _values) public onlyController returns (bytes32 lockId) {
        ...
        for(uint256 i = 0; i u003c amount; i++) {
            address from = _froms[i];
    ...
    function confirmCeilingRaise(bytes32 _lockId) public onlyCustodian {
        ...
        // overflow check
        if (newCeiling u003e= totalSupplyCeiling) {
    ...
    function lowerCeiling(uint256 _lowerBy) public onlyController {
        ...
        // overflow check
        require(newCeiling u003c= totalSupplyCeiling, "totalSupplyCeiling overflow");
        totalSupplyCeiling = newCeiling;
    ...
    /** @notice  Pass-through control of print confirmation, allowing this
      * contractu0027s custodian to act as the custodian of the associated
      * active token implementation.
      *
      * @dev  This contract is the direct custodian of the active token
      * implementation, but this function allows this contractu0027s custodian
    ...
    /** @notice  Pass-through control of custodian change confirmation,
      * allowing this contractu0027s custodian to act as the custodian of
      * the associated active token implementation.
      *
      * @dev  This contract is the direct custodian of the active token
      * implementation, but this function allows this contractu0027s custodian
